...
class Block(object):
    ...
    def __init__(self, path, name, index,eafob):

        self.path = path
        self.name = name
        self.index = index

        main_wav = Block.get_clips(path)
        wav_file= wave.open(main_wav[0], 'rb')
        n_channels, sample_width, sample_rate, _, _, _ = wav_file.getparams()

        # py_audio = pyaudio.PyAudio()
        # data = py_audio.open(format=py_audio.get_format_from_width(wav_file.getsampwidth()),
        #                      channels = wav_file.getnchannels(),
        #                      rate=wav_file.getframerate(),
        #                      output=True)

        # frames=int(0*wav_file.getframerate())
        clipslist=[]
        self.parts_length_list=[]
        # eaf_file =  [clip for clip in os.listdir(path) if clip.endswith('.eaf')]
        for index, annotation in enumerate(eafob.get_annotation_data_for_tier(name)):
            clipslist.append(annotation[1]-annotation[0])
        data, chans, width, rate, frames, clipslist = Block.merge_clips(main_wav)
